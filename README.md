...
* **자동 제어:** 온도 상한 초과 시 쿨링팬 ON, 중간값 이하 도달 시 자동 OFF / 습도 하한 이하 시 가습기 ON (건조), 중간값 이상 도달 시 자동 OFF
* **시뮬레이션 피드백:** 쿨링팬 ON → 매 사이클 온도 −0.5°C 보정 / 가습기 ON → 매 사이클 습도 +1.0% 보정 (일교차 패턴 위에 즉시 반영)
...
* **💡 Resolution:**
  ...
  * **임계값 제어 조건:**
    * 쿨링팬: 온도 ≥ 상한 → ON / 온도 ≤ 하한 → OFF / 온도 ≤ (하한+상한)/2 → 자동 OFF (과도 냉각 방지)
    * 가습기: 습도 ≤ 하한 → ON (건조) / 습도 ≥ 상한 → OFF / 습도 ≥ (하한+상한)/2 → 자동 OFF (과도 가습 방지)
* **📈 성과:** 명령 유실 없는 신뢰성 보장, 중간값 자동 OFF로 불필요한 제어 루프 방지, 이력 DB 기록으로 감사 추적 가능
...
| GET | `/api/device-control/state` | ✅ ADMIN | 기기 현재 ON/OFF 상태 조회 |
...
* **기기 피드백:** 서버에서 제어 명령 수신 시 센서 값에 즉시 반영
  * 쿨링팬 ON → 매 사이클마다 온도 −0.5°C 보정 (최저 18°C 하한)
  * 가습기 ON → 매 사이클마다 습도 +1.0% 보정 (최고 90% 상한)
  * 쿨링팬/가습기 OFF → 보정 없이 일교차 패턴으로 자연 복귀
...
최근 업데이트 2026.04.07 — README V2.1.0
